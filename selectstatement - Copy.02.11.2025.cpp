...
namespace csvquery {
    ...
    {
        queue = std::make_unique<boost::lockfree::spsc_queue<QList<csv::CSVRow>, boost::lockfree::capacity<64>>>();
    ...
    */
    ...
    {
        /*
        QList<csv::CSVRow> rows;
        rows.reserve(BATCH_ROWS);
        csv::CSVRow row;

        while (left_file2->readRow(row)) {
            ++NUMBER_OF_ROWS_IN_CSV;
            rows.append(row);  // ensure CSVRow has value semantics!

            if (rows.size() >= BATCH_ROWS) {
                while (!queue->push(std::move(rows))) {
                    if (query_done) goto cleanup;
                    QThread::msleep(10);
                }
                rows.clear();
                rows.reserve(BATCH_ROWS);  // preserve capacity
            }
    ...
    cleanup:
        ...
        read_done.store(true, std::memory_order_release); // signal consumer that producer is done
        */

        csv::CSVRow row;
        QList<csv::CSVRow> batch;
       ...
       // unsigned int row_count = 0;
        ...
        while (left_file2->readRow(row)) {
            if (done_consuming.load(std::memory_order_acquire) || canceled.load())
                break;
            ...
            // Wait if paused (UI is viewing current results)
            while (paused.load(std::memory_order_acquire)) {
                if (done_consuming.load() || canceled.load()) break;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
            }

            // Once a batch fills, push it
            if (batch.size() >= BATCH_ROWS) {
                
                // Wait if queue full
                while (!queue->push(std::move(batch))) {
                    std::this_thread::yield();
                    if (done_consuming.load() || canceled.load())
                        break;
                }
                
                // Reinitialize batch after move
                batch = QList<csv::CSVRow>();
                batch.reserve(BATCH_ROWS);
            }
        }

        // Push any leftover rows
        if (!batch.empty() && !done_consuming.load() && !canceled.load()) {
            queue->push(std::move(batch));
        }

        done_producing.store(true, std::memory_order_release);
        //////qDebug() << "Number of rows read by reader: " << row_count;
    }
    ...
    std::optional<QList<QStringList>> SelectStatement::select_with_no_join()
    {
        ...
        try {
            QList<csv::CSVRow> batch;
            //unsigned int row_count = 0;
            while (!done_producing.load(std::memory_order_acquire) || !queue->empty() || !reserve_batch.isEmpty()) {
                if (canceled.load()) break;

                if (!queue->empty() || !reserve_batch.isEmpty()) {

                    
                    if (!reserve_batch.isEmpty()) {
                        batch = reserve_batch;
                    }
                    else{
                        if (!queue->pop(batch)) {
                    ...
                    //foreach (auto& csvrow, batch) {
                    while(!batch.isEmpty()){
                        csv::CSVRow csvrow = batch.takeFirst();

                        QStringList row;
                        row.reserve(10);
                        for (csv::CSVField& field : csvrow) {
                            row.append(QString::fromStdString(field.get<std::string>()));
                        } 

                        //qDebug() << "processing row "<< ++row_count;
                        if (row.count() == 1 && row.at(0).trimmed() == "") {
                            qDebug() << "row size: " << row.size() << " row:" << row;
                            continue;
                        }
                        else {
                    ...
                    reserve_batch = batch;
                    //if (done_consuming.load()) break;
                    //////qDebug() << "Batch size after loop: " << batch.size();
                    
                    if (stop) { break; }
                    //else { reserve_batch = QList<csv::CSVRow>(); }
                }
                else {
                    //////qDebug() << "yielding...";
                    std::this_thread::yield();
                    //////qDebug() << "yielding done.";
                }
            }

            if (!has_group_by && !is_aggregation) { // query processing done if not an aggregation query
                done_consuming.store(true, std::memory_order_release);
            }
