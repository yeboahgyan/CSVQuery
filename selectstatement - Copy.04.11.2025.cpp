...
namespace csvquery {
    ...
    */
    ...
    std::shared_ptr<QHash<QString, QStringList> > SelectStatement::build_index2(const std::shared_ptr<CSVFile2>& rhs, const int& column_index)
    {
        std::shared_ptr<QHash<QString, QStringList> > index = std::make_shared<QHash<QString, QStringList> >();
        csv::CSVRow csvrow;

        qDebug()<<"rhs file: "<<rhs->get_token().string_value <<", at end of file?: "<<rhs->end_of_file();

        unsigned int rhs_rows_read = 0;
        unsigned int failed_read_count = 0;
        while (!rhs->end_of_file()) {

            if (!rhs->readRow(csvrow)) {
                ++failed_read_count;
                std::this_thread::yield();
                continue;
            }
        ...
        qDebug() << "fail read count: " << failed_read_count;
        return index;
    }
    ...
    std::optional<QList<QStringList>> SelectStatement::select_with_inner_join2()
    {
        ...
        if (join_paginate) {
            ...
            QMap<QString, QStringList> data_rows = outstanding_data_rows;

            auto rhs_rows = data_rows[join_files_list["right"]];

            //rhs_row_it = (join_paginate) ? rhs_row_it : rhs_rows.begin();

            for (; rhs_row_it != rhs_rows.end(); ++rhs_row_it) {
                stop = process_data(outstanding_data_rows);
                ...
                if (stop) {
                    ...
                    if (paused.load()) {
                        join_paginate = true;
                        //reserve_batch = std::move(batch);
                        outstanding_data_rows = data_rows;
                    }
                    else {
                        join_paginate = false;
                    }
        ...
        try {
            ...
            for (;;) {
                ...
                while (!batch.empty()) {
                    ...
                    if (query_lookup_index2->contains(row[query_index])) {
                        QStringList rhs_rows = (*query_lookup_index2)[row[query_index]];
